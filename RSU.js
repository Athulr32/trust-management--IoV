...
//For RSU
let privKeyRsu =
  "2670dcc196727cd22823dc575e7c50e3a6e48b80b1b5a2e213242a2c3ab628b8";
...
let publicKeyRsuHex =
  "03d47b61fcc919803a211309b1c2aa50e68afae3753a35e511f95b328b5ba344e0";
...
// Now apply keccak
const addressRsu = keccak256(
  Buffer.from(publicKeyUncompressedRsu, "hex"),
).slice(64 - 40);
...
function callFunctionInContract(contractFunction) {
  ...
  contractFunction.estimateGas({ from: addressRsu }).then((gasAmount) => {
    ...
    web3.eth.getTransactionCount(addressRsu).then((_nonce) => {
      //this will generate Nonce
...
app.use("/register", (req, res) => {
  ...
  const msgAuthenticity = secp256k1.ecdsaVerify(
    realSignObj,
    realMsg,
    realPubKey,
  );
...
app.use("/verifyVehicle", async (req, res) => {
  ...
  if (isVehicleVerified) {
    const getLocationFunction =
      contract.methods.getTrustValueAndLocation(address);
...
app.use("/updateTrustValue", async (req, res) => {
  ...
  if (msgAuthenticity && isVehicleVerified) {
    if (trustValue === false) {
      const getLocationFunction = contract.methods.updateTrustValue(
        address,
        -20,
      );
