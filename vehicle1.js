...
//For vehicle
let privKeyHex =
  "cd9a22bb430b4e172ddef03638d565221d315835f888aaa47e4946ec1dc4e2f6";
...
function input() {
  rl.question(
    "Enter 1 to Send Message to Nearby vehicle About an event\nEnter 2 to send a request to vehicle for info\n ",
    async function (res) {
      if (res === "1") {
        ...
        rl.question("What information you want", async (info) => {
          ...
          const address = keccak256(
            Buffer.from(publicKeyUncompressed, "hex"),
          ).slice(64 - 40);
          ...
          if (msgAuthenticity) {
            ...
            const trustRes = await fetch(
              "http://localhost:3012/updateTrustValue",
              {
                method: "POST",
                body: JSON.stringify({
                  trust: true,
